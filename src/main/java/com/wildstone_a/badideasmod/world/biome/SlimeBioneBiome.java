...
public class SlimeBioneBiome extends BADIDEASMODElements.ModElement {
	@ObjectHolder("badideasmod:slimebione")
	public static final CustomBiome biome = null;
	public SlimeBioneBiome(BADIDEASMODElements instance) {
		super(instance, 58);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					...
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(Blocks.SLIME_BLOCK.getDefaultState(),
							Blocks.SLIME_BLOCK.getDefaultState(), Blocks.SLIME_BLOCK.getDefaultState())));
			setRegistryName("slimebione");
	...
	static class CustomTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
		...
		public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldgen, Random rand, BlockPos position, MutableBoundingBox bbox) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == Blocks.SLIME_BLOCK.getDefaultState().getBlock() || ground == Blocks.SLIME_BLOCK.getDefaultState().getBlock())
							&& (ground2 == Blocks.SLIME_BLOCK.getDefaultState().getBlock()
									|| ground2 == Blocks.SLIME_BLOCK.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), Blocks.SLIME_BLOCK.getDefaultState(), bbox);
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, Blocks.SLIME_BLOCK.getDefaultState(), bbox);
								...
								for (Direction Direction : Direction.Plane.HORIZONTAL) {
									if (rand.nextInt(4 - hlevel) == 0) {
										...
										setTreeBlockState(changedBlocks, world, position.add(dir.getXOffset(), height - 5 + hlevel, dir.getZOffset()),
												Blocks.SLIME_BLOCK.getDefaultState(), bbox);
									}
								}
							}
						}
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.SLIME_BLOCK.getDefaultState().getBlock()
					|| blockType == Blocks.SLIME_BLOCK.getDefaultState().getBlock() || blockType == Blocks.SLIME_BLOCK.getDefaultState().getBlock()
					|| blockType == Blocks.SLIME_BLOCK.getDefaultState().getBlock();
		}
